...
def cal_perplexity(dist, idx=0, beta=1.0):  # dist是第i个点与其他点之间的距离（行向量）
    ...
    if sum_prob < 1e-12:
        ...
        # log(Per) = -sum(P_ij*log(P_ij))
        #          = sum(P_ij * (dist*beta + log(sum_prob)))
        #          = sum(P_ij * dist * beta) + log(sum_prob)
        #          = log(sum_prob) + sum(P/sum_prob * dist * beta)
        #          = log(sum_prob) + beta /sum_prob * sum(prob * dist)
        if sum_prob != 0:
            perp = np.log(sum_prob) + beta * np.sum(dist * prob) / sum_prob  # 求熵公式
            prob /= sum_prob  # 归一化求P_ij
...
def seach_prob(x, tol=1e-5, perplexity=30.0):  # 目标困惑值为30.0
    ...
    for i in range(n):
        ...
        perp_diff = perp - base_perp  # 计算已有熵和期望值之间的差距，即交叉熵
        tries = 0
